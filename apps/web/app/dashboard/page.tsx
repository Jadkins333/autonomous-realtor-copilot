...
export default function DashboardPage() {
  ...
  return (
    <SiteShell>
      {!tourDismissed ? (
        <Card className="mb-4" data-testid="tour-card">
          ...
          <div className="mt-3">
            <Button onClick={dismissTour} variant="outline" data-testid="dismiss-tour-btn">
              Dismiss tour
            </Button>
          </div>
        </Card>
      ...
      <Card className="mt-4" data-testid="sources-health-card">
        ...
        {sources === null ? (
          <p className="text-xs text-muted-foreground" data-testid="sources-loading">Loading…</p>
        ) : (
          <>
            {driftSources.length > 0 && (
              <div className="mb-3 rounded-lg border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-xs text-amber-300" data-testid="drift-alert">
            ...
            <div className="flex flex-wrap gap-2 text-xs">
              {okCount > 0 && (
                <Badge className="bg-green-500/20 text-green-200" data-testid="ok-badge">{okCount} ok</Badge>
              )}
              {partialCount > 0 && (
                <Badge className="bg-amber-500/20 text-amber-200" data-testid="partial-badge">{partialCount} partial</Badge>
              )}
              {failedCount > 0 && (
                <Badge className="bg-red-500/20 text-red-200" data-testid="failed-badge">{failedCount} failed</Badge>
              )}
              {staleSources.length > 0 && (
                <Badge className="bg-orange-500/20 text-orange-300" data-testid="stale-badge">{staleSources.length} stale</Badge>
              )}
              {sources.length === 0 && (
                <span className="text-muted-foreground" data-testid="no-sources">No sources configured.</span>
              )}
            </div>
          </>
        )}
      ...
      <Card className="mt-4" data-testid="metrics-card">
