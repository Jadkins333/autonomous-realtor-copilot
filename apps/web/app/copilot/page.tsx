    ...
    <SiteShell>
      <Card className="mb-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {COPILOT_COMMANDS.map((cmd) => (
            <Button key={cmd} variant="outline" onClick={() => setQuery(cmd)}>
              {cmd}
            </Button>
          ))}
        </div>
      ...
      <Card>
        ...
        <div className="space-y-3">
          {messages.length === 0 ? <p className="text-sm text-muted-foreground">Run a command to start.</p> : null}
            ...
            >
              ...
              >
                ...
                {item.payload?.trace?.selected_agent ? (
                  <div className="mt-2">
                    <Badge>agent: {item.payload.trace.selected_agent}</Badge>
                  </div>
                ) : null}
                {item.payload?.trace ? (
        ...
        {latestAssistantPayload?.data ? (
          <div className="mt-3">
            <ProvenanceDrawer
              ...
              inputs={
                latestAssistantPayload?.data?.inputs || {
                  missing_inputs: latestAssistantPayload?.missing_inputs || []
                }
              }
              provenance={
                {
                  metric_provenance: latestAssistantPayload?.data?.provenance || {},
                  trace: latestAssistantPayload?.trace || {}
                }
              }
            />
          </div>
      ...
      <Card className="mt-4">
        ...
        <ul className="space-y-2 text-sm">
          {agents.map((agent) => (
            <li key={agent.key} className="rounded-lg border bg-card p-2">
