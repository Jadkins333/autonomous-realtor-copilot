...
describe("api proxy route", function () {
  ...
  it("forwards GET query and strips hop headers", async function () {
    ...
    const req = new NextRequest(
      "http://localhost/api/proxy/sources/status?x=1",
      {
        method: "GET",
        headers: {
  ...
  it("strips content-encoding and transfer-encoding response headers", async function () {
    ...
    const mod = await import("./route");
    const req = new NextRequest("http://localhost/api/proxy/healthz", {
      method: "GET"
    });
  ...
  it("encodes path segments before forwarding", async function () {
    ...
    const mod = await import("./route");
    const req = new NextRequest("http://localhost/api/proxy/raw?q=a%2Fb", {
      method: "GET"
    });
    ...
    expect(fetchMock.mock.calls[0][0]).toBe(
      "http://api.internal/spaces%20and%2Fslash/100%25?q=a%2Fb"
    );
  });
  ...
  it("handles undefined path arrays by proxying to API root", async function () {
    ...
    const mod = await import("./route");
    const req = new NextRequest("http://localhost/api/proxy?x=1", {
      method: "GET"
    });
  ...
  it("forwards HEAD requests without a request body", async function () {
    ...
    const res = await mod.HEAD(req, {
      params: { path: ["sources", "status"] }
    });
  ...
  it("forwards OPTIONS requests with body when present", async function () {
    ...
    const res = await mod.OPTIONS(req, {
      params: { path: ["outreach", "drafts"] }
    });
  ...
  it("returns 503 json when upstream fetch fails", async function () {
  ...
  it("returns 503 timeout json when upstream aborts", async function () {
    ...
    const mod = await import("./route");
    const req = new NextRequest("http://localhost/api/proxy/slow", {
      method: "GET"
    });

    const res = await mod.GET(req, { params: { path: ["slow"] } });

    expect(res.status).toBe(503);
    await expect(res.json()).resolves.toMatchObject({ error: "proxy_timeout" });
  });
});
