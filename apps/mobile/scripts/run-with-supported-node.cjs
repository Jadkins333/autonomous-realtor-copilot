...
if (!command) {
  console.error("[mobile validate] Missing command to execute.");
  process.exit(1);
}
...
if (shouldUseNode20 && !fs.existsSync(node20Path)) {
  console.error(
    `[mobile validate] Node ${process.versions.node} is unsupported for this command and Node 20 is not installed at ${node20Path}.`
  );
  process.exit(1);
}
